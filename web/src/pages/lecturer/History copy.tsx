        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="inline-flex rounded bg-white p-1">
              ...
              <button onClick={() => setGroupBy("status")} className={`px-3 py-1 text-sm ${groupBy === "status" ? "bg-gray-100 font-semibold text-gray-900" : "text-gray-600"}`}>Status</button>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <select multiple value={selectedModules} onChange={(e) => setSelectedModules(Array.from(e.target.selectedOptions).map((o) => o.value))} className="rounded-md border px-3 py-1 text-sm text-gray-900 bg-white">
              {moduleOptions.map((m) => <option key={m} value={m}>{m}</option>)}
            </select>

            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-md border px-3 py-1 text-sm text-gray-900 bg-white">
              <option value="all">All statuses</option>
              <option value="active">Active</option>
              <option value="closed">Closed</option>
            </select>

            <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="rounded-md border px-3 py-1 text-sm text-gray-900 bg-white">
              <option value="all">Any time</option>
              <option value="today">Today</option>
              <option value="week">Last 7 days</option>
            </select>

            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search title, module, id" className="rounded-md border px-3 py-1 text-sm text-gray-900 bg-white" />
          </div>
        </div>
      </div>
                  ...
                  <div className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4 min-w-0">
                          <button onClick={() => toggleGroup(headerKey)} className="text-left">
                            <div className="text-sm font-semibold text-gray-900 truncate">{moduleCode} <span className="text-xs text-gray-600">({items.length} sessions)</span></div>
                            <div className="text-xs text-gray-600 truncate">Active: {activeCount} • Total submissions: {totalSubs}</div>
                          </button>
                        </div>
                    ...
                    <ul className="divide-y divide-stroke-subtle">
                      {items.map((session) => (
                        <li key={session.id} className="flex flex-col gap-3 px-6 py-3 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="text-sm font-semibold text-gray-900 truncate">{session.title || session.id}</p>
                            <p className="text-xs text-gray-600">{session.createdAt}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="rounded-md border border-gray-200 px-3 py-1 text-xs text-gray-600">{session.stats?.submissionsCount ?? 0} submissions</span>
                            <span className={`rounded-md px-3 py-1 text-xs font-medium ${session.isActive ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"}`}>{session.isActive ? "Active" : "Closed"}</span>
                            <Link to={`/history/${session.id}`} className="text-sm font-semibold text-indigo-600 hover:underline">View / Export</Link>
                          </div>
                        </li>
                      ))}
                    </ul>

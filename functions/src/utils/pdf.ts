...
export async function toPdfBuffer({ session, rows }: PdfInput): Promise<Buffer> {
...
interface ModulePdfInput {
  moduleStats: Record<string, any>;
  topAbsentees?: Record<string, any>[];
  sessions?: Record<string, any>[];
}

export async function toModulePdfBuffer({ moduleStats, topAbsentees = [], sessions = [] }: ModulePdfInput): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 48, size: "A4" });
    const chunks: Buffer[] = [];
    ...
    if (!topAbsentees.length) {
      ...
      topAbsentees.forEach((s: any, idx: number) => {

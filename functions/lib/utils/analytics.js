...
exports.recomputeModuleStats = recomputeModuleStats;
const firestore_1 = require("firebase-admin/firestore");
const emptyBuckets = () => ({ "0-1": 0, "1-3": 0, "3-5": 0, "5-10": 0, ">10": 0 });
function weekKey(d) {
...
async function recomputeModuleStats(db, days = 30) {
    ...
    for (const sessionDoc of sessionsSnap.docs) {
        ...
        if (median !== null)
            m.medians.push({ sessionId: sessionDoc.id, median, day, hour: createdAt.getHours() });
        Object.keys(buckets).forEach((k) => {
            m.buckets[k] += buckets[k];
        });
    ...
    for (const [moduleId, data] of modules.entries()) {
        ...
        const medianCheckin = data.medians.length
            ? (data.medians.map((x) => x.median).sort((a, b) => a - b)[Math.floor(data.medians.length / 2)])
            : null;
        const checkinCurveTotal = (Object.values(data.buckets).reduce((s, v) => s + v, 0) || 1);
        const checkinCurve = {};
        Object.keys(data.buckets).forEach((k) => {
            const val = Number(data.buckets[k] || 0);
            checkinCurve[k] = Math.round((val / checkinCurveTotal) * 10000) / 100; // percent
        });
        // compute a few quick insights
        const lowestSlot = Object.entries(data.heatmap || {}).reduce((acc, [k, v]) => {
            const vt = v;
        ...
        const fastestSession = (data.medians || []).reduce((acc, item) => {
            if (!item || item.median == null)
                return acc;
            if (!acc || item.median < acc.median)
                return { sessionId: item.sessionId, median: item.median };
            return acc;
        }, null);
        const out = {
            moduleId,
        ...
        const sessionsOrdered = (data.sessionList || []).slice().sort((a, b) => +a.createdAt - +b.createdAt);
        const studentMap = {};
        ...
        rosterSnap.forEach((r) => {
            ...
            if (sn)
                rosterMap[sn] = { name: d.name || "", surname: d.surname || "", email: d.email || "" };
        });
        for (const s of sessionsOrdered) {
            ...
            for (const r of s.attendanceRows || []) {
                ...
                if (r.submittedAt)
                    studentMap[sn].lastSeenAt = r.submittedAt;
                // prefer attendance-provided names if present
                if (!studentMap[sn].name && r.name)
                    studentMap[sn].name = r.name;
                if (!studentMap[sn].surname && r.surname)
                    studentMap[sn].surname = r.surname;
                if (typeof r.minutes === "number")
                    studentMap[sn].minutes.push(r.minutes);
            }
            ...
            for (const sn of Object.keys(studentMap)) {
                if (!presentSet.has(sn))
                    studentMap[sn].appearances.push(false);
            }
        }
        ...
        for (const [sn, sdata] of Object.entries(studentMap)) {
            const appearances = sdata.appearances || [];
